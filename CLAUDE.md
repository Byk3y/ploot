...
- **Self-review before declaring done.** A green build only proves the syntax compiles. After any non-trivial change, *read your own diff*. Trace the new code paths against their callers. Walk through edge cases mentally — empty inputs, repeated invocations, reverse toggles, late-evening timestamps, DST. Most of the bugs the owner has caught are bugs you'd have spotted in a 60-second self-read. Do that read before you write the "build's clean, please test" message.
- **Don't reflexively spawn review agents.** Calling a `code-reviewer` or `Explore` agent after every change burns tokens and time and almost always returns the same conclusion you'd reach yourself by reading the diff. Spawn an agent for review only when (a) the change touches **security-critical territory** (auth, RLS, payments, secrets, crypto), (b) you've made an **architectural decision you're genuinely uncertain about** and want a second opinion, (c) the change spans **many files / unfamiliar parts of the codebase** where a fresh agent's exploration is faster than you re-reading every dependent module, or (d) the **owner explicitly asks** for a review. For routine work — a screen redesign, a helper rename, a copy edit, a single-screen bug fix — read your own diff. That's the review.
